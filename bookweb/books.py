...
class ListView(MethodView):
    def get(self, page=1):
        ...
        book_source_genre = Book.objects(is_active=True).aggregate({"$unwind": "$genre"},
                                                                   {"$group": {"_id": "$genre", "count": {"$sum": 1}}})

        return render_template('books/list.html', books=books, book_src=book_source, book_src_g=list(book_source_genre),
...
class BookSourceListView(MethodView):
    def get(self, name, page=1):
        ...
        book_source = Book.objects(is_active=True).aggregate({"$group": {"_id": "$_cls", "count": {"$sum": 1}}})
        book_source2 = Book.objects(is_active=True).aggregate({"$unwind": "$genre"},
                                                              {"$group": {"_id": "$genre", "count": {"$sum": 1}}})
        return render_template('books/list.html', books=books, book_src=list(book_source),
                               book_src_g=list(book_source2), comment_count=get_comments_count())
...
class GenreListView(MethodView):
    def get(self, name, page=1):
        ...
        book_source = Book.objects(is_active=True).aggregate({"$unwind": "$genre"},
                                                             {"$group": {"_id": "$genre", "count": {"$sum": 1}}})
        return render_template('books/list.html', books=books, book_src=book_source2, book_src_g=book_source,
                               comment_count=get_comments_count())
...
class DetailView(MethodView):
    ...
    def drawChart(self, id):
        ...
        my_plot_div = plot(
            [Scatter(x=x, y=joy, name='joy'), Scatter(x=x, y=fear, name='fear'), Scatter(x=x, y=anger, name='anger'),
             Scatter(x=x, y=anticipation, name='anticipation'), Scatter(x=x, y=disgust, name='disgust'),
             Scatter(x=x, y=negative, name='negative'), Scatter(x=x, y=positive, name='positive'),
             Scatter(x=x, y=sadness, name='sadness'), Scatter(x=x, y=surprise, name='surprise'),
             Scatter(x=x, y=trust, name='trust')]
            , output_type='div')
...
def sentimentAnalyse(myContent):
    ...
    surprise = {}
    trust = {}
    dictOfEmotion = {'joy': joy, 'anger': anger, 'anticipation': anticipation, 'disgust': disgust, 'fear': fear,
                     'negative': negative, 'positive': positive, 'sadness': sadness, 'surprise': surprise,
                     'trust': trust}
    listOfEmotion = ['joy', 'anger', 'anticipation', 'disgust', 'fear', 'negative', 'positive', 'sadness', 'surprise',
                     'trust']
    temp = 0
    index = 0
