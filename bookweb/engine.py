    ...
    def compute_similarity(self, kernel='cosine'):
        ...
        for bk in Book.objects(is_active=True):
            ...
            if tag_emb is not None:
                ...
                row[self.get_result_col_name()] = np.average(np.ravel(similarity_mat),
                                                             weights=[x * y for x in source_weights for y in
                                                                      tag_weights])
            else:
...
class Aggreagator(Composite):
    ...
    def compute_similarity(self, kernel='cosine'):
        ...
        result_df[self.get_result_col_name()] = result_df[
            [layer.get_result_col_name() for layer in self.children]].mean(axis=1)
...
class Recommendation(object):
    ...
    def get_n_similar_books(self, n, kernel='cosine'):
        ...
        return result_df.nlargest(n, self.model.get_result_col_name()), result_df.nsmallest(n,
                                                                                            self.model.get_result_col_name())
